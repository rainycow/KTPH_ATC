# -*- coding: utf-8 -*-
"""
Model Developed on Wed May 22 13:47:07 2019

@author: MAL_Desktop12
"""
...
# Prediction Function
def ATC_predictor(input_path):
    """
    ATC Predictor is to generate an output file with corresponding ATC codes for input data
    
    input_path: the filepath of input data
    """
    
    
    # Below allows GPU to be use
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    session = tf.Session(config=config)
    
    # Load input data
    data = pd.read_csv(input_path, header = 0, na_values = ["", "-", "."], encoding = 'latin-1') 
    TEXT_COLUMN = data.columns[0]  # Ensure only one column in the input data
    ...
    data2 = pd.merge(data, lookup, how='left', left_on=['PREDICTED_ATC_INDEX'], right_on=['INDEX'])
